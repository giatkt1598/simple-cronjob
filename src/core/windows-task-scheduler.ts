...
export class WindowsTaskScheduler implements TaskScheduler {
  async reconcile(
    ...
    if (process.platform !== "win32")
      throw new Error("WindowsTaskScheduler can only run on Windows.");
    ...
    const wanted = new Set(
      jobs
        .filter(
          (job) =>
            job.enabled &&
            (!job.stopAt || parseStartAt(job.stopAt).isAfter(now)),
        )
        .map((job) => `${TASK_PREFIX}${job.id}`),
    );
    for (const taskName of existing)
      if (!wanted.has(taskName)) await this.delete(taskName);
    for (const job of jobs) {
      if (job.enabled && (!job.stopAt || parseStartAt(job.stopAt).isAfter(now)))
        await this.createOrUpdate(job, projectRoot, nodePath);
    }
  }
  ...
  private async createOrUpdate(
    ...
    const temporaryDirectory = join(
      tmpdir(),
      `simple-cronjob-${job.id}-${process.pid}`,
    );
    const xmlPath = join(temporaryDirectory, `${job.id}.xml`);
    await mkdir(temporaryDirectory, { recursive: true });
    await writeFile(
      xmlPath,
      `\ufeff${createTaskXml({ action, user, parallel: job.parallel, startAt: job.startAt })}`,
      "utf16le",
    );
    try {
      await execFileAsync(
        "schtasks.exe",
        ["/Create", "/XML", xmlPath, "/TN", taskName, "/F"],
        { windowsHide: true },
      );
    } finally {
  ...
  private async listManagedTasks(): Promise<string[]> {
    ...
    return stdout
      ...
      .filter((name): name is string => Boolean(name?.startsWith(TASK_PREFIX)));
  }
}

function createTaskXml(input: {
  action: string;
  user: string;
  parallel: boolean;
  startAt?: string;
  ...
  const configuredStart = input.startAt
    ? parseStartAt(input.startAt)
    : undefined;
  const start = configuredStart?.isAfter(nextMinute)
    ? configuredStart
    : nextMinute;
